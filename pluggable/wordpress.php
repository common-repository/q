 ...
 */
    function q_get_gallery_images( $post = null, $size = null, $limit = null, $field = null ){
        ...
        if ( ! $post || ! is_object( $post ) ) { echo 'kicked'; return false; }
...
if ( !function_exists( 'q_user_is_administator' ) ) 
{
/**
 ...
 * @since 0.1
 */
    function q_user_is_administator() {
        $current_user = wp_get_current_user();
        $current_user_role = q_get_roles_by_user_id( $current_user->ID );
        return ( ( $current_user_role && $current_user_role[0] == 'administrator' ) ? true : false );
    }
}
 ...
 * 
 * @uses        get_option()
 * @uses        q_template_warning()
 * @since       0.1
 * @return      Mixed  
 */
function q_get_template_part( $include, $setting_title = '', $setting = true )
    {
        ...
        if ( $setting === true && $include ) { // check to see if setting is active and include path passed ##
            ...
            if ( file_exists( q_get_option("path_child").'library/'.$include ) ) { // load child over parent ##
            ...
            } elseif ( file_exists( q_get_option("path_parent").'library/'.$include ) ) { // load from parent ##   
                ...
                q_template_warning( $include, $setting_title, $setting );
    ...
    function q_theme_data( $refresh = false ) 
    {
       ...
       if ( ! get_option( 'q_theme_data' ) ) { 
           ...
           if(function_exists( 'wp_get_theme' )){
               ...
               $q_theme_data = get_theme_data( TEMPLATEPATH . '/style.css');
               #$theme_version = $theme_data['Version'];
           }    
