...
class WeDevs_Settings_API {
    ...
    function admin_init() {
        ...
        foreach ( $this->settings_sections as $section ) {
            if ( false === get_option( $section['id'] ) ) { // does the option already exist ? ## 
                if ( isset( $section['dont_add_option'] ) ) {
                    // nada ##
                } else {
            ...
            if ( isset($section['desc']) && !empty($section['desc']) ) {
                $section['desc'] = '<p class="description">'.$section['desc'].'</p>';
    ...
    function callback_checkbox( $args ) {
        ...
        $html .= sprintf( '<label for="%1$s[%2$s]"> <span class="description">%3$s</span></label>', $args['section'], $args['id'], $args['desc'] );
...
<?php
                        ...
                        // no form on this page please ##
                        if ( isset( $form['hide_form'] ) && $form['hide_form'] === true ) {
                            
                            if ( isset( $form['callback'] ) && $form['cb'] ) {
                                
                                #echo "<h3>{$form['title']}</h3>";
                                
                                $q_support = new Q_Support;
                                call_user_func( array( $q_support, $form['callback']) );
...
<?php
                        ...
                        // yes - form on this page please ##
                        } else {

?>
                        <form method="post" action="options.php" id="q_support">
                            ...
                            <div style="padding-left: 10px">
                                <?php submit_button( $form['submit_text'] ); ?>
                            </div>
