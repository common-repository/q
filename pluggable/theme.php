...
{
    /**
    * add q_theme option to activate Q theme elements
    * 
    * @since       1.2.0
    * @return      void
    */
    function q_theme()
    {
        ...
        // add Q_THEME constant to activate theme elements ##
        if ( ! defined ( 'Q_THEME' ) ) { define( 'Q_THEME', true ); }
        
    }
}


if ( ! function_exists( 'q_load_theme_textdomain' ) )
{
    /**
    * load theme and child theme text-domains
    */
    function q_load_theme_textdomain() {

        if ( defined ( 'Q_THEME' ) ) {

            if ( is_dir( q_get_option("path_parent").'library/language' ) ) { load_theme_textdomain( 'q-parent', q_get_option("uri_parent").'library/language/' ); }
            if ( is_child_theme() ) { load_child_theme_textdomain( 'q-child', q_get_option("uri_child").'library/language/' ); }
...
if ( ! function_exists( 'q_template_warning' ) ) 
{
/**
 * Return a warning about a missing template or inactive template feature
 * 
 * @since   1.0
 */
    function q_template_warning( $include, $setting_title, $setting )
    {

        global $q_options;
        if ( $include && q_user_is_administator() && $q_options->framework_warning === TRUE ) {

            if ( $setting === false ) { // check to see if setting is active ##

                $warning = $setting_title.' ';
                $action = __("inactive", "q-framework").' - <a href="'.get_admin_url().'themes.php?page=framework" title="'.__("Edit Framework Options", "q-framework").'">'.__("Edit Options", "q-framework").'</a>';

            } else { // template part missing / wrongly named ##

                $warning = __("Template missing", "q-framework");
                $action = $include;

            }

            // compile ##
            $return = '<div class="error"><strong>'.$warning.'</strong> '.$action.'</div>';

            // apply filters ##
            $return = apply_filters( 'q_template_warning', $return );
...
*/
    ...
    <?php
               ...
               if ( q_user_is_administator() && $q_options->framework_warning === TRUE ) { // user is admin ##

                   $message = ( array_key_exists( $widget, $widgets ) )? __( "empty", "q-framework" ) : __( "error", "q-framework" ) ;
                   if ( !current_theme_supports('widgets') ) { $message == __( "not supported!", "q-framework" ); }
    ?>
                   <div class="error"><strong>Widget <?php echo $message; ?>:</strong> <?php echo $widget_name ? $widget_name : $widget ; ?></div>
    <?php   
               }
           }
       }
 ...
 */
    function q_plugin_warning( $title, $plugin, $path )
    {
        ...
        global $q_options;
        if ( $plugin && q_user_is_administator() && $q_options->framework_warning === TRUE ) {
       ...
       if ( $menu ) {
           ...
           if ( !isset($nav_menu) || $nav_menu === false ) { // built it ##
               ...
               if ( has_nav_menu( $menu ) ) { // menu found ##
                   ...
                   // echo warning ##
                   echo q_wp_nav_menu_warning( $menu );
 ...
 */
    function q_wp_nav_menu_warning( $menu ){
       ...
       global $q_options;
       if ( $menu && q_user_is_administator() && $q_options->framework_warning === TRUE ) {
 ...
 * @since           0.1
 */
    function q_add_theme_support( $support, $action = true )
    {

       // load global variable ##
       global $q_options;

       if ( $support && is_object( $q_options ) ) { // check to see if $support passed ##
